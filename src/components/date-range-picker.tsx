...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const getPresetRange = (presetName: string): DateRange => {
    ...
    switch (preset.name) {
      ...
      case "nextWeek":
        ...
        to.setDate(first + 7 + 6);
        break;
      case "nextMonth":
        // must start from the first day of next month
        from.setMonth(from.getMonth() + 1, 1);
        // must end at the last day of next month
  ...
  const checkPreset = useCallback((): void => {
    for (const preset of PRESETS) {
      ...
      const normalizedRangeFrom = new Date(range.from || Date.now());
      normalizedRangeFrom.setHours(0, 0, 0, 0);
      const normalizedPresetFrom = new Date(
        presetRange.from?.setHours(0, 0, 0, 0) ?? Date.now()
      );
                ...
                <div className="flex gap-2">
                  <DateInput
                    ...
                    onChange={(date) => {
                      ...
                      onUpdate?.({ range: { from: date, to: toDate } });
                    }}
                  />
                  ...
                  <DateInput
                    ...
                    onChange={(date) => {
                      const fromDate =
                        date < (range?.from ?? new Date()) ? date : range.from;
                      setRange((prevRange) => ({
                        ...prevRange,
                        from: fromDate,
                        to: date,
                      }));
                      onUpdate?.({ range: { from: fromDate, to: date } });
                    }}
                  />
                </div>
                {rangeCompare != null && (
